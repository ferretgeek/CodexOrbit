...
GitHub Actions 工作流使用 GitHub 的 `actions/*`、`github/codeql-action`、
`actions/attest` 和 Microsoft 的 `microsoft/setup-msbuild`。这些工具只在 CI
中运行，不进入应用发布包；工作流引用固定到已审查的完整提交 SHA。
