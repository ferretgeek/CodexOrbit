...
The upstream App Server interface is evolving. Codex Orbit v3.2.0 is verified
...
show an unknown-publisher prompt on first launch. Verify the SHA-256 before
running it. If the EXE is moved later, toggle autostart off and on so the saved
path is updated.
...
UI settings and a packaged-runtime cache are stored under:
...
notifications, local-log parsing, quota thresholds, plan mapping, reset
tracking, and window placement.
