...
8. 协议夹具不得包含真实邮箱；账户响应测试应确认只缓存必要字段。
9. GitHub Actions 必须固定到完整提交 SHA，且 `build` 与 `codeql` 检查通过。
...
确定性测试，避免只能依赖真实账户验证。发布打包应保持文件顺序和时间戳稳定；
只有在源码与编译工具链一致时才比较二进制结果，不宣称跨编译器版本必然一致。
